import tkinter as tk
from tkinter import messagebox
from datetime import datetime 

def xu_ly_du_lieu():
    # Lấy dữ liệu từ ô nhập và xóa khoảng trắng thừa ở 2 đầu
    mssv = o_nhap_ma_sv.get().strip()
    ho_ten = o_nhap_ho_ten.get().strip()
    
    # 3. Tương tác Terminal: Lấy thời gian hiện tại và in ra
    thoi_gian_hien_tai = datetime.now().strftime("%H:%M:%S")
    print(f"[{thoi_gian_hien_tai}] Đã nhận: MSSV {mssv} - Tên: {ho_ten}")
    
    # 4. Thông báo Popup: Kiểm tra nếu có ô bị để trống
    if mssv == "" or ho_ten == "":
        messagebox.showerror("Chú ý", "Vui lòng không để trống thông tin sinh viên!")
        return # Dừng hàm tại đây, không chạy các lệnh phía dưới nữa
    
    # 2. Ràng buộc dữ liệu: Kiểm tra MSSV có phải là số không
    if not mssv.isdigit():
        nhan_ket_qua.config(text="Lỗi: Mã sinh viên phải là các chữ số!", fg="red")
        return # Dừng hàm tại đây
        
    # --- Nếu vượt qua hết các kiểm tra trên, tức là thành công ---
    
    # Cập nhật Label thành công
    nhan_ket_qua.config(text=f"Thành công: Đã nhận dữ liệu của {ho_ten}", fg="green")
    
    # 1. Xóa trắng 2 ô nhập liệu
    o_nhap_ma_sv.delete(0, tk.END)
    o_nhap_ho_ten.delete(0, tk.END)

# --- THIẾT LẬP CỬA SỔ CHÍNH ---
root = tk.Tk()
root.title("Quản lý Sinh viên - UHL")
root.geometry("400x300") # Cân chỉnh lại kích thước cho gọn
root.columnconfigure(1, weight=1)

# --- PHẦN GIAO DIỆN ---
tk.Label(root, text="Mã sinh viên:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
o_nhap_ma_sv = tk.Entry(root)
o_nhap_ma_sv.grid(row=0, column=1, padx=10, pady=10, sticky="ew")

tk.Label(root, text="Họ và tên:").grid(row=1, column=0, padx=10, pady=10, sticky="w")
o_nhap_ho_ten = tk.Entry(root)
o_nhap_ho_ten.grid(row=1, column=1, padx=10, pady=10, sticky="ew")

# Nút bấm
nut_xac_nhan = tk.Button(root, text="Xác nhận điểm danh", command=xu_ly_du_lieu, bg="#0078D7", fg="white")
nut_xac_nhan.grid(row=2, column=0, columnspan=2, pady=10, padx=10, sticky="ew")

# Nhãn hiển thị kết quả ban đầu
nhan_ket_qua = tk.Label(root, text="Hệ thống sẵn sàng", font=("Arial", 10, "italic"))
nhan_ket_qua.grid(row=3, column=0, columnspan=2, pady=20)

root.mainloop()